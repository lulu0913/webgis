<template>
  <div>
    <form id = "app">
        <h2 class="form-signin0-heading">用户信息修改</h2>
        <br>
        <div id="mid">
        <p v-if="seen_1">账户名：</p><input v-if="seen_1" class="form-control" type="text" placeholder="请输入账户名"  name="account" v-model="account">
        <el-row v-if="seen_1" ><el-button @click="submitQuery" value="查询用户" class="submitbutton_query" type="warning">查询用户</el-button></el-row>
        ...
        <el-row v-if="seen_2" ><el-button @click="submitEdit" value="提交修改" class="submitbutton_edited" type="warning">提交修改</el-button></el-row>
        </div>
    </form>
  </div>
